classdef ODE45 < SSP_Tools.Integrators.Integrator
	...
	methods
		...
		function [u_next,varargout] = advance(obj,u,t,dt)
			...
			[t_, u_] = ode45(odefun, [t,t+dt], u, odeset(obj.ode_opts));
			u_ = u_(end,:);
			u_next = u_';
		...
		function id_string = repr(obj)
			...
			for i=1:numel(opt_fields)
				...
				if ~isempty(value)
					if isnumeric(value)
						opt_string = [ opt_string, sprintf('%s=%g ', field, value) ];
					elseif ischar(value)
