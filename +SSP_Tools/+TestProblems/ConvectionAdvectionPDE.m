classdef ConvectionAdvectionPDE < SSP_Tools.TestProblems.PDE

	properties		
		
		f_burgers = @(u) -0.5*u.^2;
		f_burgers_em = @(u) max(abs(u));
		
		f_advection = @(u) -u;
		f_advection_em = @(u) 1.0;
		
		% For this problem, we need two discretizer objects. The 'discretizer'
		% property inherited from SSP_Tools.TestProblems.PDE is unused.
		burgers_discretizer = []
		advection_discretizer = []
	...
	methods
		...
		function obj = ConvectionAdvectionPDE(varargin)
			p = inputParser;
			p.KeepUnmatched = true;
			p.addParamValue('discretizer', []);
			p.addParamValue('burgers_discretizer', []);
			p.addParamValue('advection_discretizer', []);
			p.addParamValue('xi_1', []);
			p.addParamValue('xi_2', []);
			p.parse(varargin{:});
			
			params = p.Unmatched;
			params.name = sprintf('Convection-Advection Equation u_t + %g u_x + %g (0.5*u^2)_x = 0', p.Results.xi_1, p.Results.xi_2);
			params.domain = [-1, 1];
			...
			% Set our discretizers.
			if ~isempty(p.Results.burgers_discretizer)
				obj.set_burgers_discretizer(p.Results.burgers_discretizer)
			end
			
			if ~isempty(p.Results.advection_discretizer)
				obj.set_advection_discretizer(p.Results.advection_discretizer)
			end
			
			% This is unused
			p.Results.discretizer;
		...
		function set_burgers_discretizer(obj, discretizer)
			...
			discretizer.f = @obj.f_burgers;
			discretizer.em = @obj.f_burgers_em;
			discretizer.left_boundary = obj.left_boundary;
			discretizer.right_boundary = obj.right_boundary;
			obj.burgers_discretizer = discretizer;
		end

		function set_advection_discretizer(obj, discretizer)
			% Initializes the discretizer. Right now this only
			% accepts an SSP_Tools.SpaceMethod object, but this
			% behavior can easily be changed.
			discretizer.f = @obj.f_advection;
			discretizer.em = @obj.f_advection_em;
			discretizer.left_boundary = obj.left_boundary;
			discretizer.right_boundary = obj.right_boundary;
			obj.advection_discretizer = discretizer;
		end
		
		function set_discretizer(obj, discretizer)
			% Unused
			fprintf('set_discretizer() is unused in this object');
		end
		...
		function RHS = RHS(obj, u,t)
			...
			tmp_discretizer1 = obj.advection_discretizer;
			u_x_advection = obj.discretize(u, t, @tmp_discretizer1.L);
		
			tmp_discretizer2 = obj.burgers_discretizer;
			u_x_burgers = obj.discretize(u, t, @tmp_discretizer2.L);
			
			RHS = obj.xi_1*u_x_advection + obj.xi_2*u_x_burgers;
		end
...
%  			end
		...
		function data = export(obj)
			...
			data.burgers_discretizer = obj.burgers_discretizer.get_repr();
			data.advection_discretizer = obj.advection_discretizer.get_repr();
