function [wp, is_values] = weno9(u_values, u_idx, n, epsilon, p)
	...
	for j=u_idx
		...
		for k=1:numel(window_indices)
			wp(window_indices(k), j_absidx) = wp(window_indices(k), j_absidx) + interp_coeffs(k);
		end
		%
		% I'm unable to vectorize this operation because each column of 'interp_coeffs' contains
		% an update for an overlapping region of of the matrix 'wp'. I could just go column-by-
		% column like this
		%  
		% for k=1:m
		% 	wp(window_indices(:,k),j_absidx) = wp(window_indices(:,k), j_absidx) + interp_coeffs(:,k);
		% end
		%
		% but I have found that it's slightly faster to flatten everything out and update things 
